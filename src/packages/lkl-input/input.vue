<template>
  <div class="lkl-input">
    <div class="lkl-input-input">
      <input class="lkl-input-input-input" required ref="input" :style="{ fontSize, color }" v-model="text" type="text" :pattern="pattern" @keyup.enter="onEnter" @blur="onBlur" @input="onInput" />
      <div v-show="text.length === 0 && !triggleInput" :style="{ fontSize: fontSize, color: placeholderColor }" class="lkl-input-input-placeholder">{{ placeholder }}</div>
    </div>
    <div v-if="clean" v-show="text.length > 0" class="lkl-input-clear" @touchStart="onClean" @mousedown.prevent="onClean">
...
@Component
export default class LklSearchField extends Vue {
  @Prop({ default: '' }) public pattern!: string;
  @Prop({ default: '请输入' }) public placeholder!: string;

  @Prop({ default: 'var(--font14)' }) public fontSize!: string;
